...
from app import pl_bone,  df_to_table, histogram_equalization, get_pl_image, DICOM_heatmap
...
#######################################
dc_ViewPosition_opacity =count_two_cols(df_target_path, 'ViewPosition','LungOpacity','total')
# Select by Age
...
#######################################
dataframes = {'By Gender': dc_gender_opacity,
    'By View Position':dc_ViewPosition_opacity, 'By Age':dc_age_opacity}
...
def update_graph(app2dropdown):
    ...
    trace1 = go.Bar(x=df_normal['Group'], y=df_normal['total'],text=df_normal['total'], textposition = 'auto', name='Normal')

    trace2 = go.Bar(x=df_opacity['Group'], y=df_opacity['total'], text=df_opacity['total'], textposition = 'auto', name='Opacity')
